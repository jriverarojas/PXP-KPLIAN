...
?>
<script>
Phx.vista.proceso_contrato=Ext.extend(Phx.gridInterfaz,{

	constructor:function(config){
		this.maestro=config.maestro;
    	//llama al constructor de la clase padre
		Phx.vista.proceso_contrato.superclass.constructor.call(this,config);
		this.init();
	...
	Atributos:[
       	...
       	},
		{
   			config:{
   				...
   				store: new Ext.data.JsonStore({
					...
					totalProperty: 'total',
					fields: ['id_uo','nombre','ci'],
					// turn on remote sorting
					remoteSort: true,
					baseParams:{par_filtro:'nombre#ci'}
				}),
   				valueField: 'id_uo',
   				displayField: 'nombre',
   				gdisplayField:'desc_uo',//dibuja el campo extra de la consulta al hacer un inner join con orra tabla
   				tpl:'<tpl for="."><div class="x-combo-list-item"><p>{nombre}</p><p>CI:{ci}</p> </div></tpl>',
   				hiddenName: 'id_uo',
   				forceSelection:true,
   				typeAhead: true,
       			triggerAction: 'all',
		...
		
		{
   			config:{
   				...
   				valueField: 'id_representante_legal',
   				displayField: 'nombre_completo1',
